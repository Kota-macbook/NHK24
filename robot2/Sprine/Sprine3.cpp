...
void GetCombinations(int PointIDNum,int Combinations[]){
    int i;
    ...
    Combinations[0]=1;
    Combinations[1]=PointIDNum;
    Combinations[PointIDNum]=1;
    Combinations[PointIDNum-1]=PointIDNum;

    int HalfPointNum_1=PointIDNum/2+1;
    int Numerator = PointIDNum;
    int Denominator=1;
    for(i=2;i<HalfPointNum_1;i++){
        Numerator-=1;
        Denominator+=1;
        Combinations[i]=Combinations[i-1]*Numerator/Denominator;
        Combinations[PointIDNum-i]=Combinations[i];
    }
...
int main(void){
    ...
    FILE* ReadTxt=fopen("data3.txt","r");
    ...
    //二項係数の計算
    GetCombinations(PointIDNum,Combinations);
    printf("Combinations\n");
    for(i=0;i<PointNum;i++){
        printf("%3dC%3d:%d\n",PointIDNum,i,Combinations[i]);
    }
    ...
    FILE* WriteTxt=fopen("write3.txt","w");
    ...
    while(ReadT<TimeEn){
        ...
        XT=0;
        YT=0;
        for(i=0;i<=PointIDNum;i++){
            FuncT=pow(ReadT,i)*pow(1-ReadT,PointIDNum-i);
            XT+=FuncT*ConstsX[i];
            YT+=FuncT*ConstsY[i];
        }
//-------------------------------------------------
