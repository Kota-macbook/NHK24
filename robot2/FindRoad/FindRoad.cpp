...
int Astar(char* MapDefault,int* Goals,int SizeX,int SizeY,float OpenNode[][3],int OpenNodeNum,char MapJudge);
...
int main(void){
	...
	char* MapDefault=(char*)malloc(SizeY*SizeX*sizeof(char));
...
//IMPORT MAP HERE!
	...
	int* Goals=(int*)malloc((SizeX*SizeY+1)*sizeof(int)/2); 
	int GoalMoveNum;

    auto StartTime = std::chrono::system_clock::now();
	x= Astar(MapDefault,Goals,SizeX,SizeY,OpenNode,OpenNodeNum,MapJudge);
    auto EndTime = std::chrono::system_clock::now();
    std::chrono::duration<double, std::milli> UsedTime = EndTime - StartTime;
    printf("UsedTime=%lf[ms]\n",UsedTime);

	GoalMoveNum=Goals[0];
	for(i=0;i<GoalMoveNum;i++){
		printf("%d(%d,%d)\n",2*i+1,Goals[2*i+1],Goals[2*i+2]);
	}
	...
	if(CheckRoop){
		...
		*(ReMap+(int)(ReGoalY*ReSizeX+ReGoalX))=(char)127;
		...
		while(FindGoalsRoop){
			...
			Goals[2*count+1]=GoalsX-1;
			Goals[2*count+2]=GoalsY-1;
...
/*
	if(count!=0){
		int GoalNodes[count][2];
		ReadGoalsX=ReGoalX;
		ReadGoalsY=ReGoalY;
		i=0;
		GoalNodes[count-1-i][1]=ReGoalY;
		GoalNodes[count-1-i][0]=ReGoalX;
		for(i=1;i<count;i++){
			GoalsX=*(MapLogX+(int)(ReadGoalsY*ReSizeX+ReadGoalsX));
			GoalsY=*(MapLogY+(int)(ReadGoalsY*ReSizeX+ReadGoalsX));
			ReadGoalsX=GoalsX;
			ReadGoalsY=GoalsY;
			GoalNodes[count-1-i][1]=GoalsY;
			GoalNodes[count-1-i][0]=GoalsX;
		}
